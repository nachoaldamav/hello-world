...
This environment can be applied to all your components or a set of components via the command `bit env set <pattern or component_id> development/envs/node-env`, or under the variants of your `workspace.json` file.
...
Run the command `bit env set <"pattern" or component_id> development/envs/node-env` where the pattern is a glob pattern to select multiple components. This will put the env
...
#### Alternative Usage
Under the **variant** section of your `workspace.json` file choose which components you want to have the custom environment set. You can find the id of the extension in the `node-env.aspect.ts` file.

```json
{
  "teambit.workspace/variants": {
    "{ui/**}, {pages/**}": {
      "development/envs/node-env": {}
    }
  }
}
```
...
Bit uses ESLint to lint your components. You can add your own rules in the `node-env.main.runtime` file.
...
Bit uses Prettier to format your components. You can add your own rules in the `node-env.main.runtime` file.
...
Override the default dependencies in the `node-env.main.runtime` file to include react types of a different version for example.
...
Extend the `node-env.preview.runtime` file when you want to add your own customizations only for previewing in the Bit UI.
